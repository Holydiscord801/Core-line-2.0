...
function extractVerbPhrase(recommendation) {
    if (!recommendation)
        return null;
    ...
    while (words.length > 0 && stop.has(words[0].toLowerCase()))
        words.shift();
    if (words.length === 0)
        return null;
    return words.slice(0, 4).join(' ').toLowerCase();
}
function fallbackVerb(status) {
    switch (status) {
...
router.get('/', async (req, res) => {
    try {
        ...
        for (const row of seqResult.data || []) {
            ...
            if (jid && !seqMap.has(jid))
                seqMap.set(jid, row);
        }
        const fuMap = new Map();
        for (const row of fuResult.data || []) {
            const jid = row.job_id;
            if (jid && !fuMap.has(jid))
                fuMap.set(jid, row);
        }
        const signalMap = new Map();
        for (const row of signalResult.data || []) {
            const jid = row.related_job_id;
            if (jid && !signalMap.has(jid))
                signalMap.set(jid, row);
        }
...
router.get('/:id/full', async (req, res) => {
    try {
        ...
        if (jobResult.error) {
            ...
            res.status(500).json({ error: jobResult.error.message });
            return;
        }
        ...
        const contacts = (contactsResult.data || []).map((jc) => {
            const contact = jc.v2_contacts;
            const contactOutreach = allOutreach.filter((o) => o.contact_id === contact.id);
        ...
        const followups = (followupsResult.data || []).map((f) => {
            ...
            const contactOutreach = allOutreach.filter((o) => o.contact_id === f.contact_id);
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (data && parsed.data.fit_score !== undefined && parsed.data.fit_score >= 70 && !data.job_description && data.url) {
            (async () => {
                try {
                    const jdResult = await fetchJobDescription(data.url);
                    if (jdResult.text) {
...
router.post('/fetch-all-jds', async (req, res) => {
    try {
        const userId = req.userId;
...
router.post('/generate-all-cover-letters', async (req, res) => {
    try {
        const userId = req.userId;
...
router.post('/:id/fetch-jd', async (req, res) => {
    try {
        ...
        const updates = { job_description: result.text };
