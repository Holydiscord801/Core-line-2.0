...
router.get('/sequences', async (req, res) => {
    try {
        ...
        const sequenceIds = sequenceList.map((s) => s.id);
        ...
        for (const a of attempts || []) {
            const sid = a.sequence_id;
            if (!sid)
                continue;
            if (!attemptsBySeq.has(sid))
                attemptsBySeq.set(sid, []);
        ...
        const result = sequenceList.map((s) => {
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (response_at !== undefined)
            updateData.response_at = response_at;
