...
async function fetchGreenhouse(url) {
    ...
    if (!m)
        return { text: null, source: 'greenhouse', error: 'Could not parse Greenhouse URL' };
    ...
    try {
        ...
        if (!res.ok)
            return { text: null, source: 'greenhouse', error: `API ${res.status}` };
...
async function fetchLever(url) {
    ...
    if (!m)
        return { text: null, source: 'lever', error: 'Could not parse Lever URL' };
    const [, company, jobId] = m;
    ...
    try {
        ...
        if (!res.ok)
            return { text: null, source: 'lever', error: `API ${res.status}` };
        const data = await res.json();
...
async function fetchAshby(url) {
    ...
    if (!html)
        return { text: null, source: 'ashby', error: 'Failed to fetch page' };
    const m = html.match(/<script id="__NEXT_DATA__"[^>]*>([\s\S]*?)<\/script>/);
    if (!m)
        return { text: null, source: 'ashby', error: 'No __NEXT_DATA__ found' };
    try {
        const data = JSON.parse(m[1]);
        const job = data?.props?.pageProps?.jobPosting;
        if (!job)
            return { text: null, source: 'ashby', error: 'No jobPosting in page data' };
...
async function fetchBambooHR(url) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const text = raw.includes('<') ? stripHtml(raw) : raw;
            if (text)
                return { text, source: 'bamboohr' };
        }
    }
    catch { /* fall through to generic */ }
    return fetchGeneric(url, 'bamboohr');
}
async function fetchGeneric(url, source = 'generic') {
    const html = await fetchPage(url);
    if (!html)
        return { text: null, source, error: 'Failed to fetch page' };
...
export function isBlacklistedJobUrl(url) {
    if (!url)
        return false;
...
export async function fetchJobDescription(url) {
    if (!url)
        return { text: null, source: 'none', error: 'No URL provided' };
    const u = url.toLowerCase();
    ...
    if (u.includes('myworkdayjobs.com'))
        return fetchGeneric(url, 'workday');
    if (u.includes('smartrecruiters.com'))
        return fetchGeneric(url, 'smartrecruiters');
    if (u.includes('jobvite.com'))
        return fetchGeneric(url, 'jobvite');
    if (u.includes('icims.com'))
        return fetchGeneric(url, 'icims');
    return fetchGeneric(url);
}
//# sourceMappingURL=jd-scraper.js.map