...
function detectThemes(jd) {
    ...
    if (/\b(lead|director|vp|head of|manage.*team|team.*lead|build.*team|org)\b/.test(t))
        themes.push('leadership');
    if (/\b(ai|machine learning|\bml\b|llm|generative ai|artificial intelligence)\b/.test(t))
        themes.push('ai_ml');
    if (/\b(scale|scaling|million users?|high.traffic|platform|infrastructure|architecture)\b/.test(t))
        themes.push('scale');
    if (/\b(transform|moderniz|digital transform|legacy|migration|re-architect)\b/.test(t))
        themes.push('transformation');
    if (/\b(revenue|p&l|commercial|business impact|profit|growth|retain)\b/.test(t))
        themes.push('revenue');
    if (/\b(startup|fast.paced|high.growth|early.stage|series [ab]|seed)\b/.test(t))
        themes.push('startup');
    if (/\b(engineer|architect|backend|frontend|cloud|kubernetes|devops)\b/.test(t))
        themes.push('technical');
    if (/\b(product|roadmap|strategy|cross.functional|stakeholder)\b/.test(t))
        themes.push('product');
    return themes;
}
/**
 ...
 */
function extractHighlights(resumeText) {
    if (!resumeText)
        return [];
    ...
    return sentences
        ...
        if (/\d/.test(s))
            score += 2;
        if (/\$\d+|\d+[km]\b|\d+%|\d+x\b/i.test(s))
            score += 4;
        if (/\b(led|built|launched|scaled|managed|drove|delivered|grew|increased|reduced|generated|retained)\b/i.test(s))
            score += 2;
        if (/\b(revenue|users?|engineers?|team|growth|retention|platform|org|officers?)\b/i.test(s))
            score += 1;
        return { s, score };
    })
        .filter(x => x.score >= 2)
...
function themeScore(highlight, themes) {
    const h = highlight.toLowerCase();
    let score = 0;
    for (const theme of themes) {
        switch (theme) {
            case 'leadership':
                if (/led|manag|team|org|report|direct|engineers?/.test(h))
                    score += 3;
                break;
            case 'ai_ml':
                if (/ai|ml|machine|model|data|intelligent/.test(h))
                    score += 3;
                break;
            case 'scale':
                if (/scale|million|users?|platform|traffic|growth/.test(h))
                    score += 3;
                break;
            case 'transformation':
                if (/transform|modern|legacy|migrat|rebuild|rebuilt/.test(h))
                    score += 3;
                break;
            case 'revenue':
                if (/revenue|\$|million|billion|retained|commercial/.test(h))
                    score += 3;
                break;
            case 'startup':
                if (/startup|launch|zero|built|founded|greenfield/.test(h))
                    score += 3;
                break;
        }
    }
...
function opening(profile, job, themes) {
...
function bodyParagraph(highlights, themes, index) {
 ...
 */
export function generateCoverLetterText(profile, job) {
    ...
    if (profile.profile_data?.contact_email)
        parts.push(profile.profile_data.contact_email);
    if (profile.profile_data?.contact_phone)
        parts.push(profile.profile_data.contact_phone);
    return parts.join('\n');
}
