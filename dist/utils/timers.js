 ...
 */
export function calculateDueDate(sentDate, timerType) {
 ...
 */
export function getEscalationLevel(firstOutreachDate) {
    const bizDays = countBusinessDays(firstOutreachDate, new Date());
    if (bizDays >= ESCALATION.AUTO_ARCHIVE)
        return 'archive';
    if (bizDays >= ESCALATION.ESCALATE_CONTACT)
        return 'escalate';
    if (bizDays >= ESCALATION.SECOND_FOLLOWUP)
 ...
 */
export function getFollowupDescription(timerType, sentDate) {
