...
import type { BattlePlanData } from '../../types/index.js';
...
async function generateBattlePlanForUser(userId: string): Promise<any> {
  const today = new Date().toISOString().split('T')[0];
    ...
    // ---- Derived sets ----
    const newJobs = activeJobs.filter((j: any) => j.status === 'new');
    const staleApplied = activeJobs.filter((j: any) =>
      j.status === 'applied' && j.updated_at && j.updated_at < fiveDaysAgoISO
    );
    ...
    for (const fu of overdueFollowups) {
      const job = (fu as any).v2_jobs;
      const contact = (fu as any).v2_contacts;
    ...
    for (const o of staleOutreach) {
      const contact = (o as any).v2_contacts;
      const job = (o as any).v2_jobs;
    ...
    // ---- Build plan_data ----
    const jobs: BattlePlanData['jobs'] = activeJobs.slice(0, 25).map((j: any) => {
      let action = 'Review';
      if (j.status === 'new') action = 'Evaluate and apply';
      else if (j.status === 'applied') action = 'Follow up';
      else if (j.status === 'interviewing') action = 'Prepare for interview';
      else if (j.status === 'researching') action = 'Research and decide';
      else if (j.status === 'offer') action = 'Review offer terms';
      return { id: j.id, title: j.title, company: j.company, fit_score: j.fit_score, action };
    });

    const contactsToReach: BattlePlanData['contacts_to_reach'] = warmContacts.map((c: any) => ({
      id: c.id,
      name: c.name,
      company: c.company || '',
    ...
    if (overdueFollowups.length > 0) summaryParts.push(`${overdueFollowups.length} overdue follow-up(s)`);
    if (todayFollowups.length > 0) summaryParts.push(`${todayFollowups.length} follow-up(s) due today`);
    if (newJobs.length > 0) summaryParts.push(`${newJobs.length} new job(s) to review`);
    if (staleOutreach.length > 0) summaryParts.push(`${staleOutreach.length} outreach message(s) without a response`);
    if (staleApplied.length > 0) summaryParts.push(`${staleApplied.length} application(s) going stale`);
