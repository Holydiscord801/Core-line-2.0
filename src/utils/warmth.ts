...
export function getWarmthLevel(score: number): WarmthLevel {
...
export function getWarmthColor(score: number): string {
  if (score <= 20) return '#52525e';  // dim gray
 ...
 */
export function updateWarmthScore(currentScore: number, event: keyof typeof WARMTH_EVENTS): number {
 ...
 */
export function applyWarmthDecay(currentScore: number, lastContactedAt: Date | null): number {
  if (!lastContactedAt || currentScore <= 0) return currentScore;

  const weeksSince = Math.floor(
    (Date.now() - lastContactedAt.getTime()) / (7 * 24 * 60 * 60 * 1000)
  );

  // Grace period: no decay for first 2 weeks
  if (weeksSince <= 2) return currentScore;
 ...
 */
export function getWarmthDots(score: number): ('filled' | 'partial' | 'empty')[] {
  const dots: ('filled' | 'partial' | 'empty')[] = [];
